...
class MailerProcessor:
    ...
    def processInboxMails(self):
        ...
        cursor.execute('select "value" from app_config where name = %s ', [
                       "APP_TICKET_CODE_PREFIX"])
    ...
    def __checkIfEmailIsAssignedToTicket(self, email, conn):
        ...
        cursor.execute(
            """select id from tickets where code = %s""", [ticketCode])
    ...
    def __handleEmailValid(self, eId, email, conn) -> str:
        ...
        cursor.execute(DB_QUERY_STRING)
        cursor.execute(
            """select id,subject,content,status from tickets where code = %s""", [ticketCode])
    ...
    def __handleEmailNotFound(self, eId, email, conn) -> str:
        ...
        self.__asociateTicketWithClient(
            conn, ticketID, f"{senderMailBox}@{senderHost}", senderHost)
    ...
    def __asociateTicketWithClient(self, conn, ticketID, emailBox, senderDomain):
        cursor = conn.cursor()
        cursor.execute(DB_QUERY_STRING)
        cursor.execute("""
                INSERT INTO admin_clients_tickets (ticket_id, client_uid, created_at, updated_at) 
                select %s,i.client_uid,now(),now() from (select client_uid from admin_clients_mails_domains acmd where "domain"=%s
                union 
                select client_uid from admin_clients_mails_emails acme where email=%s
                ) i 
        """, [ticketID, senderDomain, emailBox])
        cursor.close()
    ...
    def __buildEmailTemplate(self, template_id: int, conn):
        ...
        while indexOfBlock != -1:
            ...
            retTemplate = self.__buildEmailTemplate_FindAndReplaceComponents(
                indexOfBlock, retTemplate, conn)
