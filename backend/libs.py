...
def admin_decode_auth_token(auth_token) -> tuple[str, bool]:
    ...
    try:
        # print('Token Validation with key',server.config.get('SECRET_KEY'))
        cursor.execute(
            "select user_id,token from admin_users_sessions_blacklisted where token = %s", [auth_token])
        ...
        # print(auth_token)
        payload = jwt.decode(auth_token, server.config.get(
...
def adminLoginCheck(function):
    ...
    def wrap(*args, **kwargs):
        ...
        cursor.execute(
            "SELECT id,is_su,username,email from admin_users WHERE id = %s", [response[0]])
        r = cursor.fetchone()
        request.user = User()
        request.user.id = r[0]
